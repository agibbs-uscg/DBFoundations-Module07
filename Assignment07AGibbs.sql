...
-- 2020-08-23,AGibbs,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AGibbs')
	 Begin 
	  Alter Database [Assignment07DB_AGibbs] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AGibbs;
	 End
	Create Database Assignment07DB_AGibbs;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AGibbs;
...
Select ProductName,UnitPrice From dbo.vProducts;
go

Select ProductName, Format(UnitPrice, 'C', 'en-us') As 'UnitPrice USD' 
	From dbo.vProducts
...
Select CategoryName, ProductName, Format(UnitPrice, 'C', 'en-us') As 'UnitPrice USD' 
...
Select  (DateName(mm, InventoryDate) + ', ' + DateName(YYYY, InventoryDate)) as InventoryDate, [Count] as InventoryCount
	From vInventories
Go

Select ProductName, (DateName(mm, InventoryDate) + ', ' + DateName(YYYY, InventoryDate)) as InventoryDate, [Count] as InventoryCount
...
Select Top 500000
	ProductName, (DateName(mm, InventoryDate) + ', ' + DateName(YYYY, InventoryDate)) as InventoryDate, [Count] as InventoryCount
	From dbo.vInventories
	 Join dbo.vProducts
	  on vInventories.ProductID=vProducts.ProductID
Order By ProductName,InventoryDate,[Count]
...
Select TOP 500000
		CategoryName, (DateName(mm, InventoryDate) + ', ' + DateName(YYYY, InventoryDate)) as InventoryDate, Sum([Count]) as TOTALInventoryCountByCategory
	From dbo.vCategories
		Join dbo.vProducts on vProducts.CategoryID= vCategories.CategoryID
		Join dbo.vInventories on vInventories.ProductID=vProducts.ProductID
...
Create -- drop
Function dbo.fProductInventoriesWithPreviousMonthCountsWithKPIs(@Value1 int)
 Returns Table
	As
	Return(
		Select 
